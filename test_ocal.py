...
class ocaltest(unittest.TestCase):
    def assertYMD(self, o, cal, year, mon, day, dow, msg):
        self.assertEqual(o.calendar, cal, "{} showed the wrong calendar".format(msg))
        self.assertEqual(o.year,    year, "{} in year {} gave year {}"  .format(msg, year, o.year))
        self.assertEqual(o.month,    mon, "{} in month {} gave month {}".format(msg, mon,  o.month))
        self.assertEqual(o.day,      day, "{} in day {} gave day {}"    .format(msg, day,  o.day))
        self.assertEqual(o.dow,      dow, "{} in dow {} gave dow {}"    .format(msg, dow,  o.dow))
    ...
    def test_ocal_init(self):
        ...
        self.assertYMD(o, ocal.GREGORIAN, 1995, 9, 27, 3, "init with given gregorian")


        o=ocal.ocal(year=1995, month=9, day=27)
        self.assertEqual(o.date, 49987, "init default:gregorian didn't return correct mjd date")
        self.assertYMD(o, ocal.GREGORIAN, 1995, 9, 27, 3, "init with default gregorian")
        ...
        self.assertYMD(o, ocal.JULIAN, 1995, 9, 14, 3, "init with given julian")
    ...
    def test_ocal_gregorian(self):
        ...
        self.assertYMD(o, ocal.GREGORIAN, 1995, 9, 27, 3, "init with gregorian function")

    def test_ocal_julian(self):
        o=ocal.julian(1995, 9, 14)
        self.assertEqual(o.date, 49987, "julian init didn't return correct mjd date")
        self.assertYMD(o, ocal.JULIAN, 1995, 9, 14, 3, "init with julian function")
    ...
    def test_ocal_next_dow(self):
        ...
        self.assertEqual(o.dow, 0, "went to dow 0, but made it to {} instead".format(o.dow))

        o=ocal.ocal.gregorian(2014, 6, 1)
        o.next_dow(1, 0) # advance to Sunday, but 6/1/2014 IS a Sunday
        self.assertEqual(o.get_ymd_g(), (2014, 6, 8), "Advancing to same day failed")
        ...
        self.assertEqual(o.get_ymd_g(), (2014, 5, 29), "Advancing forward 2 weeks failed")
        ...
        o.next_dow(-1, 0)
        self.assertEqual(o.get_ymd_g(), (2014, 6, 1), "Going back to yesterday failed")

        o=ocal.ocal.gregorian(2014, 6, 1)
        o.next_dow(1, 0, offset=-1)
        self.assertEqual(o.get_ymd_g(), (2014, 6, 1), '"Advancing" to self (via offset=-1) failed"')
    ...
    def test_ocal_repr(self):
        ...
        self.assertEqual(rp, exp, "repr failure. Expected '{}', got '{}'".format(exp, rp))

    def test__add(self):
        n=ocal.julian(2015, 12, 25)
        th=n+12
        self.assertYMD(th, ocal.JULIAN, 2016, 1, 6, 2, "__add from Nativity 2015")
        self.assertEqual(th.get_ymd_j(), (2016, 1, 6), "__add__ failed. Nativity+12 is {}".format(th.get_ymd_j()))

    def test__sub(self):
        th=ocal.julian(2016, 1, 6)
        n=th-12
        self.assertYMD(n, ocal.JULIAN, 2015, 12, 25, 4, "__sub__from Theophany (")
        self.assertEqual(n.get_ymd_j(), (2015, 12, 25), "__sub__ failed. Theophany-12 is {}".format(n.get_ymd_j()))

        self.assertEqual(th-n, 12, "relative difference failed. Difference between Theophany and Nativity is {} days".format(th-n))
    ...
    def test__cmp(self):
        ...
        d1+=1
        self.assertGreater(d1, d2, "failure: {} not greater than {}".format(d1, d2))
...
class ocalpascha(unittest.TestCase):
    def test_pascha(self):
        ...
        for yd in ydates:
            o=ocal.pascha(yd[0])
            self.assertEqual(o.dow, 0, "Pascha for {} not on Sunday according to o.dow")
            self.assertEqual(o.get_dow(), 0, "Pascha for year {} not on Sunday!")
            self.assertEqual(o.get_date(), yd[1], "Pascha failed for year {} ({} != {})".format(yd[0], o.get_date(), yd[1]))
